...
def run_credit_scoring_pipeline(
    ...
    *,
    ...
    The ML agent and the 3 specialist agents run independently; the Orchestrator consumes their outputs.
    """
    start = time.time()

    llm = _get_llm()
    provider = os.getenv("LLM_PROVIDER", "ollama")
    model_name = os.getenv("OLLAMA_MODEL", "llama3.1:8b") if provider.strip().lower() == "ollama" else os.getenv("OPENAI_MODEL", "gpt-4o")
